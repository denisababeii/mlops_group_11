...
class PosterClassifierService:
    ...
    def __init__(self) -> None:
        ...
        if not label_names_path.exists():
            raise FileNotFoundError(
                f"Label names file not found at {label_names_path}. Please ensure the processed data exists."
            )
    ...
    def _predict_internal(self, image: np.ndarray) -> dict[str, list[str] | list[float]]:
        ...
        print(
            f"Timing - Preprocess: {preprocess_time:.2f}s, Inference: {inference_time:.2f}s, Total: {total_time:.2f}s"
        )
